...
遍历`effectList`,对那之上的每个Fiber节点进行相应的Tag操作：`Placement` `Update` `Deletion`

2. 执行 useLayoutEffect 的销毁函数



在Layout阶段前 Mutation阶段后 会切换双缓存中的Fiber树
...
ClassComponent => 根据currentFiber树是否为空，执行componentDidMount 或 componentDidUpdate

FunctionComponent => 同步调用 useLayoutEffect 回调  调度useEffect的销毁 + 回调，在layout 阶段完成后再异步执行
...
从源码中，React在这个函数中划分了两个大的情况，一是children为string/number/Object的情况，一是children为Array的情况，因此我们也可以分为单结点diff，多结点diff 进行讨论：
...
在多次复用组件时，各个组件之间需要保持独立的data。

data是数据的构造函数，在每次调用时都会创建全新的data对象。
...
1+1+N（Browser进程 + GPU进程 - 3D绘制 + N X Renderer进程）



Browser 进程主要负责：页面进程管理、BOM管理、网络资源的管理和下载，接收Renderer提供的bitmap进行渲染等

GPU进程主要和 3D 渲染有关

Renderer进程是针对 Tab 的，非严格一一对应（每个Tab对应一个进程）



而Renderer进程下分5个线程：

1. GUI渲染线程 处理HTML DOM /  CSS - CSSOM - 进行排布计算和样式计算，最终生成渲染树，
2. JS 引擎线程 执行 JS 代码 其中可能涉及到DOM的交互，因此它要和GUI线程相互阻塞
3. 事件处理线程 用于控制事件循环的线程，管理宏任务、微任务队列，观察任务的执行情况。有事件的执行时机到了，就会把事件推入待执行队列，等到JS线程的执行栈空，就会把任务推到JS执行栈之中。
4. 定时器线程 计时、判断计时器是否到时
5. HTTP请求线程



WebWorker是主线程向进程申请多开的一个线程，它是封闭在进程之中的，而SharedWorker是在独立的进程之中管理，可被多个Renderer进程所共享
...
之前传统的方式`MultiRepo`当中，每个项目都对应单独的一个代码仓库。弊端：

- 代码复用：无法依赖共享 共用的依赖需要`npm link`，维护困难
- 版本管理:  版本不同、依赖也不自动升级
- 项目的基建：CICD流程独立，脚手架独立



MonoRepo的好处：

1. **工作流的一致性**，由于所有的项目放在一个仓库当中，复用起来非常方便。

   如果有依赖的代码变动，那么用到这个依赖的项目当中会立马感知到。

2. **项目基建成本的降低**，所有项目复用一套标准的工具和规范



实现：使用 lernajs 进行包管理 / yarn workspace

也是适合微前端应用组织的方式

独立版本模式和固定模式 -> 独立更新 / 同步更新版本号 

多包启动 / 多包GIT管理 版本管理的问题

节省重复依赖 => 重复依赖使用npm link



## NPM Yarn

Yarn 比 npm ( 初始版本 ) 优秀的原因：

1. 并行下载; npm 是顺次安装

2. 有`yarn.lock`文件，保证下载版本统一：
...
Node_modules 的目录结构也是经历了变化，从最开始的树形结构变成了扁平化的结构
...
1. BEM命名规范
   ...
   块 元素 修饰符 来表示CSS类名，使它们更加的清晰

2. postcss ——后处理器，它可接受预处理器的sass/less，或者是原生的css输入。它就像插件一样，为CSS增加额外的功能。

   比如

   - auto-prefixer

   - postcss 模组化 `postcss-modules`  / `css-loader`进行模块化也是可行
   - `postcss-preset-env` 支持未来语法等功能

   我们在webpack配置中加入postcss-loader，在postcss.config.js 引入对应的插件，在plugin中调用

   <img src="/Users/tonylee/Library/Application Support/typora-user-images/image-20211019221146445.png" alt="image-20211019221146445" style="zoom:90%;" />

3. sass less

   在工程化开发中，使用最多的应该就是Less、Sass ---- css预处理器
...
4. CSS-module 在webpack中利用css-loader,会把类名更改，并加入生成对应模组的JSON文件中，而我们在最终Webpack处理后，组件里以引入JSON对象，类名来代替直接写classname，这就做到了多个模组样式之间的隔离

5. Css in js

   本质上并不是某个库，而是一种理念。即用JS来书写和处理样式，而不是作为独立的样式文件，主要有两种方法，style属性的内联样式 和 书写类名，CSS类名附上Hash，组件挂载时插入生成的`<style>` 标签对应。

   内联样式的不好之处是处理伪类，媒体查询等

   CSS in JS 的好处是能结合React的组件状态得到动态的样式，二是以JS变量形式书写的样式，在构建之时能够去除堆积的无用的样式。
...
4. 从配置项中的 `Entry` 出发，对每个文件执行：
   ...
   - JS文件调用 parser, 进行tokenize，得到AST，从中解析依赖关系。在这个时候，loader有机会介入，对得到的AST语法树进行某种逻辑的修改。
   - 把得到的依赖关系加入到 depenency graph 之中
   - 重复本步骤，直到没有依赖为止

5. 编译之后，进入 seal 阶段，所有的文件会根据胶水代码`__webpack_require__`形成模组，之后按一定规则形成 chunk.
...
6. 文件会利用 fs 输出到 output目录，如果打开了观察模式，每一次变化就会重启 Compliation 过程，在这个过程，；

7. 如果有设置`dev-server`,则会把静态资源发送到浏览器上，并观察更新



在这个过程中，Webpack会抛出生命周期，我们可以注册钩子函数，做出一些自己的逻辑。这也就是plugin的实现。
...
Loader 本质上是一翻译作用的中间件，它能处理文件中的内容，根据预设和规则，把一种文件内容输出成另一种形式。它的本质，是AST之间的转化，由一个树状结构进行修改，生成修改后的树状结构。
...
Loader 本质上是类似流式 中间件的思想，输入`content` ，输出修改后的`content`

之后在webpack 配置项中的 `resolveLoader` 选项中注册
...
- Loader：
  ...
  - CSS-Loader ： 处理`@import`，模块化的 CSS => 把各个CSS文件的内容抽出，以字符串的形式拼接，并添加JSON类名映射
  - Style-Loader ： 生成 HTML 中的行内 `<style>`, 插入document之中
...
1. 插件和Loader构建速度性能分析插件：`speed-measurement-webpack-plugin`
2. Babel-loader 之中：
   - 去除不必要的文件解析
   - `cacheDirectory`, 在FileSystem中缓存loader执行结果
3. Webpack5 的持久性缓存：可以把缓存文件存放在文件系统中，即使退出了webpack 进程，在下次新开构建进程时可以使用上次构建的缓存

#### Source-Map

源代码到构建后代码映射

核心：mapping字段，使用base64编码，代表压缩代码，文件，源代码的位置（行、列）等， 对AST上的结点进行表示。

使用base64编码可以压缩长度 以64个字符把二进制数据进行分块和编码
...
- 资源模块的内置：raw-loader file-loader url-loader => assets-loader

- 编译的持久化缓存，改善构建的速度：Webpack会缓存打包后的文件、chunk来改善构建速度，可以选用memory cache | filesystem cache

- Tree-shaking 在webpack 4的基础之上进行改进，不只是简单找一个import变量是否出现过；webpack5的TreeShaking可以根据作用域之间的关系进行优化
...
1. 使用原生 ES Module
   ...
   在 Vite 生成的项目中，我们会在 `index.html `中引入 `type="module"`的 `<script>`，由浏览器检查之中的 `import/export`函数，去作出对应的HTTP请求，解析得到的文件，找到下一个文件，一直不断地去执行。

   和传统基于 Node.JS 的打包工具如webpack，因为这层逻辑交由了浏览器底层的C++代码去执行，因此会比webpack 更快。

2. 动态引入的文件只有在真正被需要时才会被获取，它不需经过事先的完整应用的打包流程，因此它可以瞬间起动

3. ESBuild 是基于 Go 的JavaScript处理打包工具，相比Webpack =>  express 服务器而言速度会更快
...
Parse 阶段：词法分析（Tokenize） => 语法分析 （AST转化）

Transform 阶段：基于访问者模型对AST的遍历（由一个遍历者进行AST的遍历，根据遍历到的结点类型，调用相关的插件回调）

Generate 阶段：基于转化后的AST，重新生成转化后的JS代码
...
WebAsembly 本质上是字节码 ByteCode，也就是某种语言的编译目标
...
JavaScript下载 => Parser 把 JS 代码块转化为 AST => 根据 AST, Bytecode Compiler 生成引擎能够直接阅读、执行的字节码 ByteCode

=> ByteCode会进入 Interpreter ，把它翻译成 Machine Code

=> 如果代码片段多次被执行，那么ByteCode会进入 JIT Compiler，进行优化的编译，生成更高效的Machine Code （Optimization）

=> 如果代码的变量改变，不符合JIT Compiler的假设，那么就会进入 Bailout , 把之前的优化Machine Code 废用 (Deoptimization)



如果反覆地进行 Optimization Deoptimization，那么它的成本是很大的。



WebAssembly 的前身就是`asm.js`，它是一个JavaScript 的预处理库，把我们的JS代码在实际运行之前，进行一定程度的优化。避免了反覆优化、去优化的过程



WebAssembly 是一份字节码的标准

Wasm 本质上一种底层的类汇编语言（Assembly），由其它高级语言编译生成。

大多数高级语言的编译目标不是x86 / ARM的汇编语言，而是在两者之间的Intermediate Represenatiton（IR）这个过程叫做前端

由IR编译成汇编语言叫作后端

而Wasm是一个对应虚拟架构的汇编语言，它的指令也叫做 Virtual Instructions。
...
- 根本目的：利用前端图像的可视表达，提高人对数据的认知。



- 大部分的可视化都是集成在产品里面的，并且包含更多的数据和交互
